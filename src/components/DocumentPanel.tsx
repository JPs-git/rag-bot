...
export function DocumentPanel() {
  ...
  const selectedDocument =
    state.documents.find((doc) => doc.id === selectedDocumentId) || null;
  const documentChunks = state.chunks.filter(
    (chunk) => chunk.documentId === selectedDocumentId,
  );
  const selectedChunk =
    state.chunks.find((chunk) => chunk.id === selectedChunkId) || null;
  ...
  const handleFileChange = async (
    ...
    if (
      state.documents.length + fileArray.length >
      PERFORMANCE_CONFIG.MAX_FILE_COUNT
    ) {
    ...
    try {
      ...
      if (result.skippedCount > 0) {
        message.warning(
          `${result.skippedCount} 个文件已存在，已跳过: ${result.skippedFiles.join(", ")}`,
        );
      }

      if (result.failedCount > 0) {
        const failedMessages = result.failedFiles
          .map((f) => `${f.name}: ${f.error}`)
          .join("; ");
        message.error(
          `上传失败 ${result.failedCount} 个文件: ${failedMessages}`,
        );
      }
            ...
            <div className="mt-4">
              ...
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                <div className="text-xs text-gray-400 mb-2">
                  位置: {selectedChunk.startIndex + 1} -{" "}
                  {selectedChunk.endIndex}
                </div>
